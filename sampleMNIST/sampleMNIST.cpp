    ...
    const samplesCommon::BufferManager& buffers, const std::string& inputTensorName, int *inputFileIdx) const
{
    ...
    std::vector<uint8_t> fileData(mParams.batchSize*inputH * inputW);
    uint8_t *p = fileData.data();

    for(int i=0;i< mParams.batchSize; i++)
    {
            readPGMFile(locateFile(std::to_string(inputFileIdx[i]) + ".pgm", mParams.dataDirs), p, inputH, inputW);
            p += inputH * inputW;
    }
...
//	cudaEventCreate(inputConsumed);	
    ...
    if (!context->enqueue(mParams.batchSize, buffers.getDeviceBindings().data(), stream, inputConsumed))
    {
        return false;
    }
    
 //   if(cudaEventQuery(inputConsumed) == cudaSUCCESS);
